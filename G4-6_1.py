...
class Saving_account(Account):
    ...
    def report(self): #implementing the abstract method
        ...
        lab9 = Label(report,text=self.balance,font=('Arial', 16,'bold'),fg=other_color, bg=color)
    ...
    def login_acc():
        def user_menu():
            ...
            lab1 = Label(existing_win,text=f"Welcome back {current.fname}",font=("Comic Sans MS",25,'bold'),fg=color,bg='#8dc43e'
                         ,pady=30,padx=290,activeforeground=color,activebackground='#8dc43e')
            lab1.pack()
...
def read_accounts():
    f = open('accounts.txt')
    a = f.readlines()
    a.pop(0)   #the first line of the file contains the template therefore removing the first line


    for i in a:
        content = i.split(',')
        
        if content[8] == '1' or content[8] == '2':       #content list contains [fname,lname,address,account_no,password,date,bal,strg,type]
